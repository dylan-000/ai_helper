...
def setup_chroma_db(chunks: List[Dict[str, Any]], collection_name: str = "terraria_rag", use_ollama_embeddings: bool = True, ollama_model: str = "nomic-embed-text") -> chromadb.Collection:
    """
...
def generate_response(query: str, contexts: List[str], model: str = "llama3.2:latest") -> str:
    ...
    prompt = f"""You are an assistant to a player playing Terraria. Give any important details that are required for steps that you provide.
...
def main():
    ...
    llm_model = "deepseek-r1:7b"  # Change to your preferred LLM model
    
    # 1. Load documents
    data_dir = "Model/RAG_Implementation/data"
    ...
    # 4. Example queries
    queries = [
        "What are the items that I can get fishing and turning in quests to the Angler?"
    ]
